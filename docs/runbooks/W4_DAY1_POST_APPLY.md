...
## 2. MSK 토픽 — **terraform 선언 관리** (생성=gitops `(A)`, 존재 검증=team-lead `(B)`)
> ✅ **2026-06-02 gitops 전환 완료**(라이브 입증): 9토픽은 gitops `infra/aws/dev/kafka-topics/`(terraform `kafka_topic`)가 선언 관리. **bastion 수동 `create-kafka-topics.sh`는 MSK에서 폐지**(로컬 docker-compose 전용으로만 잔존). 권위 설정 = **partitions 3 / RF 2(dev 2브로커) / min.isr 2 / retention 168h**.
- [ ] **(A, gitops)** `terraform apply`(infra → kafka-topics 순) → broker 주소 변경 시 `BootstrapBrokerStringTls` output 갱신 + gitops ConfigMap `KAFKA_BROKERS` 갱신
- [ ] **(B, team-lead)** 토픽 존재 검증(bastion SSM, TLS): `kafka-topics.sh --bootstrap-server "$BROKER" --command-config /tmp/client.properties --list` → **9토픽** 확인
- [ ] **(B)** describe로 설정 확인: `--describe --topic platform.auth.user-registered-v1` → **partitions 3 / rf 2 / min.isr 2**
> 상세·TLS·트러블슈팅: [MSK_TOPIC_SETUP](../guides/MSK_TOPIC_SETUP.md). 수동 생성은 terraform provider 접속 실패 시 폴백으로만.
