...
- **할 일**: ① **dev→main PR 생성·머지**. ② Consumer(CardsGenerated)는 D-001로 **철회**(불요). ③ **AI 카드 생성 알림 트리거 재설계**(open) — cards-generated 소비가 사라졌으므로, learning-ai 경량 알림 이벤트/별도 신호/알림 생략 중 learning-ai owner와 합의. audit/notification Consumer(W4 범위)는 유지.
- **Done**: main 머지 + S1(user-registered) E2E 확인 + 알림 트리거 방식 합의.
- **이슈**: [#30](https://github.com/team-project-final/synapse-platform-svc/issues/30)

### ✅ 무작업 확정 — learning-ai (D-001)
- Consumer(NoteCreated) main 머지 ✅, 카드 등록 HTTP. **CardsGenerated Producer 불요 → 추가 작업 없음**. DLQ 발행 로직 유지.
- 단, platform 알림 재설계(P1-1 ③)에서 경량 이벤트 발행 채택 시 그때 협의.
...
| platform-svc | dev→main PR + 알림 재설계 | ⏳ | ⏳ | ⏳ | P1 |
| learning-ai | (D-001 무작업) | — | — | — | 완료 |
| learning-card | — | ✅ | ✅ | ⏳ | 완료 |
