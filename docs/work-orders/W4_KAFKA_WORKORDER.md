...
platform-svc는 **synapse-shared Avro를 사용하지 않고** 자체 패밀리(`com.synapse.event.*`, 봉투 `time:long`·`data:bytes` 중첩)를 `PlatformAvroEvents.java`에 인라인 정의. shared(`com.synapse.*`, `time:string`)와 **봉투·네임스페이스가 불일치** → 서비스 간 Avro 상호운용에 광범위 영향. 결정 필요(택1): (a) shared가 platform 패밀리 채택 / (b) platform → shared 마이그레이션 / (c) 공존+경계 문서화. 상세: [NOTIFICATION_TRIGGER_AI_CARDS §2.2/§8.1](../designs/NOTIFICATION_TRIGGER_AI_CARDS.md). **알림 트리거 구현의 선결.**
