...
| note-updated-v1 | knowledge-svc | learning-ai, knowledge-svc (ES indexer) | NoteUpdated | ✅ |
...
## 4.1 ⚠️ 추가 발견 (05-29) — platform 스키마 패밀리 분기

본 리뷰는 synapse-shared의 8종이 **공유 계약**임을 전제했으나, 실측 결과 **platform-svc는 이 shared Avro를 사용하지 않음**. platform은 `PlatformAvroEvents.java`에 자체 인라인 스키마(`com.synapse.event.platform.*`)와 다른 봉투(`com.synapse.event.shared.CloudEventEnvelope`: `time:long`, `data:bytes` 중첩)를 정의. shared(`com.synapse.*`, `time:string`, data 없음)와 **봉투·네임스페이스 불일치**.

- 영향: shared 스키마의 "정상"이 곧 서비스 간 상호운용 보장은 아님. learning-ai/learning-card가 어느 패밀리를 쓰는지 추가 확인 필요.
- 결정 안건 **D-002**: 패밀리 정합 (shared 채택 / platform 마이그레이션 / 공존). → [W4_KAFKA_WORKORDER](../work-orders/W4_KAFKA_WORKORDER.md), [NOTIFICATION_TRIGGER_AI_CARDS §2.2](../designs/NOTIFICATION_TRIGGER_AI_CARDS.md).
