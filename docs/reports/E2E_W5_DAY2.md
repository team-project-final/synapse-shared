...
| **F9** | knowledge 인증 | 검색 `GET /api/v1/notes/search`가 platform JWT를 **401 거부**(F7과 동류 추정) | P3 검색 SLA 측정·검색 시나리오 차단 | **P2** — knowledge JWT 신원/검증 정합 확인(@knowledge) |
...
| P3 | 검색 <2s | **미측정** — knowledge 검색 401(F9) + 코퍼스 부재 | 🔴 차단 |
...
- **F9 (P2)**: knowledge 검색 JWT 401 정합 → P3 검색 SLA·검색 시나리오 선결
