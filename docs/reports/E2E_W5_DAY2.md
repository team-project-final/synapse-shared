...
Day 1 P0 2건(가입·알림 체인 전멸)을 **벤더링 정본 교체로 해소 → 라이브 재검증 완료**. 가입→게이미피케이션(W4)·전도메인 audit(W2)·알림 발행·소비(W3 알림 leg) **PASS**. 추가로 **JWT 신원 모델 버그 2건 발견·수정·검증**: F7(engagement#33, W5 신고접수 PASS — HTTP↔Kafka 신원 일치 입증) · F9(knowledge#59, 검색 인증 401 해소). platform admin 발급 부재(F8)는 기록. **W1 복습→XP→레벨업→audit 체인 PASS**(P4 ~0.67s) — 단 알림(FCM) leg는 미배선(F10). 잔여: W5 모더레이션(F8)·W1 알림 leg(F10)·W3 AI 생성 + P3 검색(F4) — 시드/외부키/owner합의 갭.
...
| **W1** | 복습 → XP → 레벨업 → 알림 | 🟢 **체인 PASS** / 🔴 알림 leg 미구현 | 경계 유저(xp=90,lvl=1) 시드 + `ReviewCompleted`(userId=90001,GOOD) 발행 → engagement XP 90→**100**·레벨 1→**2**(`Applied XP from ReviewCompleted`) → `level-up-v1` 발행 → platform `audit_logs` **LEVEL_UP/90001** 적재. **체인 ~0.67s**(P4 <10s ✅). 멱등성 구현·단위검증됨. **알림(FCM) leg는 미배선(F10)** — level-up이 알림으로 이어지지 않음 |
...
| **F10** | 게이미피케이션 알림 | **level-up → 알림(FCM) 미배선** — `GamificationKafkaProducer`는 level-up/badge 토픽만 발행(notification-send 미발행), platform은 level-up을 **audit로만** 소비(NotificationKafkaConsumer는 notification-send 전용). 라이브 확정(레벨업 후 platform 알림 처리·notification-send 흔적 0) | W1 "복습→…→알림" 마지막 단계 미완 — 사용자는 레벨업 시 알림 못 받음 | **P1** — level-up(및 badge) → notification-send 발행 경로 추가(@engagement) or platform이 level-up 소비 시 알림 트리거(@platform). 설계(E2E_SCENARIOS_W4 W1)는 의도했으나 미구현 |
...
- **F10 (P1)**: level-up(및 badge) → 알림(notification-send/FCM) 경로 추가 → W1 알림 leg 완성(@engagement or @platform)
