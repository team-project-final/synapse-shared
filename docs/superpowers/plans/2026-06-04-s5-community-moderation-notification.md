...
## Task 1: NotificationSend Avro 스키마 편입 (Task 0 정정 — platform-canonical)

> ⚠️ **Task 0 결과**: shared `NotificationSend.avsc`는 발산된 DRAFT(namespace `com.synapse.event.platform`, `eventId`/`occurredAt`/`traceparent` 누락)라 **사용 금지**. platform 소비자가 실제 쓰는 **platform-canonical 스키마**(`synapse-platform-svc/src/main/avro/platform/NotificationSend.avsc`, namespace `com.synapse.platform`)를 vendor. 선례: engagement에 이미 `platform/UserRegistered.avsc`가 동일 방식으로 vendor돼 있음.

**Files:**
- Create: `src/main/avro/platform/NotificationSend.avsc` (**platform-svc에서 복사**)

- [ ] **Step 1: 스키마 복사** — `synapse-platform-svc/src/main/avro/platform/NotificationSend.avsc`(namespace `com.synapse.platform`, 필드: eventId·tenantId·occurredAt·traceparent?·userId·notificationType·channels·title·body·emailSubject?·emailHtmlBody?·data)를 engagement `src/main/avro/platform/NotificationSend.avsc`로 **동일 복사**.
- [ ] **Step 2: Avro 생성 확인**

Run: `./gradlew generateAvroJava`
Expected: BUILD SUCCESSFUL. `build/generated-main-avro-java/com/synapse/platform/NotificationSend.java` 생성(namespace `com.synapse.platform`).
...
import com.synapse.platform.NotificationSend; // Task0 V2: platform-canonical namespace
import org.apache.avro.specific.SpecificRecord;
...
```
> 테스트(Step 3)의 import도 `com.synapse.platform.NotificationSend`로 맞춘다(생성 클래스 namespace).
...
```
> Task0 V3: `SharedContent.getOwnerId()`·`Group.getOwnerId()`(Long) **이미 존재** — 도메인 변경 불필요. soft-delete 인지 `findByIdAndDeletedAtIsNull` 사용.
...
- [ ] **Step 4: ReportController가 tenantId 전달** (Task0 V4 정정) — `@CurrentTenant` 애너테이션은 **없음**. 핸들러는 이미 `@AuthenticationPrincipal Jwt jwt`를 받으므로 `String tenantId = CurrentTenant.resolve(jwt);`(static, 없으면 `"default"` 반환)로 추출해 `moderationService.moderate(reportId, request, tenantId)` 호출. `import com.synapse.engagement.shared.CurrentTenant;`
